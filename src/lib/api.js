...
if (!baseURL) {
  ...
  const msg = "VITE_API_URL no está seteada en producción";
  console.error("❌", msg);
  // Podrías forzar fallo de build si querés:
  // throw new Error(msg);
}
...
// Errores → toast automáticos solo para red/5xx
api.interceptors.response.use(
  (res) => res,
  (err) => {
    ...
    // Si es error de cliente (4xx), lo maneja el componente → sin toast global
    if (status && status >= 400 && status < 500) {
      return Promise.reject(err);
    }

    // Otros errores (red/5xx) → sí toast
    toast.error(msg);
    return Promise.reject(err);
  }
);
